...
import swmutsel.model.MCMCTreeHessian;
...
public class Run {
    ...
    private void start(String... arguments) {
        ...
        if (args.fix.contains("all")) {
            CoreUtils.msg("Fixing all parameters. Calculating log-likelihood only!\n");
            calculate(args);
        } else {
            optimise(args);
        }
    ...
    private void optimise(Arguments args) {
        ...
        while (!converged) {
            ...
            int restarts = 1;
            if (iteration % args.robustFitnessEstimationInterval == 0) {
                restarts = args.numberOfOptimRestarts;
                if (restarts != 1 && args.robustFitnessEstimationInterval != 1) {
            ...
            if (!converged) {
                ...
                ArgumentWriter.writeFile(args, out);
                try {
        ...
        // We've converged
        String out = args.identifier + "_MLE.txt";
        ...
        try {
            ...
            BufferedWriter writer = new BufferedWriter(new FileWriter(new File(out), true));
            writer.write("Site log-likelihood and fitness:");
            writer.newLine();

            // TODO: output all sites, not just unique patterns!!
            for (int site : args.sites.columnKeySet()) {
                writer.write(String.format("%s,%.7f,%s", site, siteLogLikelihood.get(site), Doubles.join(",", args.fitnesses.get(site).get())));
                writer.newLine();
            }
            ...
            throw new RuntimeException("Run.optimise ERROR: Could not write to " + out);
        }


        // TODO: optimise clade model here, if we need to
        if (args.cladeModel.size() > 1) {
            optimiseCladeModel(args, runner);
        }

        if (args.hessian) {
            int siteCount = 0;
            for (int n : args.patternWeight.values()) siteCount += n;
            MCMCTreeHessian hessianCalculator = new MCMCTreeHessian(args.tree, args.fitnesses, mutation, args.penalty, siteCount, runner);
        }
    ...
    private void optimiseCladeModel(Arguments args, Runner runner) {
        ...
        Pair<Map<Integer, Double>, List<FitnessStore>> results = runner.optimiseFitness(args.tree, mutation, args.fitnesses, args.penalty, args.cladeModel, args.numberOfOptimRestarts);
    ...
    private void calculate(Arguments args) {
        Runner runner = getRunner(args);

        SwMut mutation = new SwMut(args.kappa, args.scaling, args.pi);
        mutation.build();

        Map<Integer, Double> siteLogLikelihoods = runner.getLogLikelihood(args.tree, mutation, args.fitnesses, args.penalty);

        double sum = CoreUtils.sum(siteLogLikelihoods.values());


        String out = args.identifier + "_CALCULATE.txt";
        ...
        try {
            ...
            //todo output all sites, not just patterns
            for (int site : args.sites.columnKeySet()) {
                writer.write(String.format("%s,%.7f,%s", site, siteLogLikelihoods.get(site), Doubles.join(",", args.fitnesses.get(site).get())));
                writer.newLine();
            }
            ...
            throw new RuntimeException("Run.calculate ERROR: Could not write to " + out);
        }
    ...
    private Runner getRunner(Arguments args) {
        Runner runner;
        ...
        if (args.distributed) {
            ...
            MatrixArrayPool.treeSize = args.tree.getInternalNodeCount();
